...
router.get("/", (req, res) => {
  ...
  const header = {
    headers: { Authorization: "Bearer " + process.env.YELP_ACCESS },
  };
...
router.get("/:id", (req, res) => {
  ...
  axios.get(url, header).then(async function (response) {
    // handle success
    // create variables
    ...
    if (restaurantAndReviews === null) {
      ...
      res.render("restaurants/show.ejs", {
        restaurant: currentRestaurant,
        restId: restaurantIdentifier,
        reviews: restaurantAndReviews.reviews,
        userData: res.locals.user,
      });
    }
  });
